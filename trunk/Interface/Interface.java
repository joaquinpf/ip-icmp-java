...
// Ver Linux: \linux-2.6.11.6\include\linux\if.h  por contenido a definir
  ...
  String hwAddr;       // Direccion de hardware (por ejemplo Ethernet, en este caso genérico)
  ...
  // Crea la interfaz
  public Interface(String nm, IpAddress addr, Mask msk, Integer mtu /*,Link k*/ ) throws NodeException {
    name = nm;
    ipAddr = addr;
    netMask = msk;
    MTU = mtu;
    sendQueue = new Utils.Queue();
    System.out.println("Creada Interfaz:"+ name+" IP: "+ipAddr.toString()+" Mask: "+netMask.toString()+" MTU:  "+MTU);
  }


  // Recepcion de un frame desde el link. Aqui se desencapsula (por ejemplo se saca el encabezamienyo ethernet
  // y en funcon del tipo de frame se invoca a arp, ip, etc. La desencapsulacion en realidad
  // no se hace porque ese nivel no interesa.
  // Se pasa a IP (por ejemplo) el datagram mas info acerca de pro que interfaz (esta) llego
  public void receive(byte[]b) {
    ...
    int y = b.length - 2;
    rec = new byte[y];
    for(int i=0;i<y-2;i++) rec[i]= b[i+2];
    // Invoca al protocolo correspondiente. No seria necesario codigo para cada protocolo, siempre
    ...
    switch (proto) {
      case NetworkProtocols.PROTO_IP:
        ...
        ((ProtocolInterface)(NetworkProtocols.getProtocol(new Integer(proto)))).addRem(n);
        //ProtocolInterface p = (ProtocolInterface)(Interfaces.getProtocol(new Integer(proto)));
        //p.addRem(rec);
        //IP.addRem(rec);
        break;
  ...
  // En nuestro caso, solo interesa el campo que indica qué protocolo es el que se encapsula en 
  // el frame del link layer
  public void send(int protocol, byte[] frame) {
    byte[] nf = new byte[frame.length + 2];
    int l;
    l = frame.length;
    for(int i=2;i<l;i++) nf[i]= frame[i-2];
  ...
  public String toString() {
    String rt = name+" IP:"+ipAddr.toString()+" Mask:"+netMask.toString()+" MTU:"+MTU;
    return rt;
  }
  public String getName() {return name;}
  
  public IpAddress getIPAddress() { return ipAddr;}
  }
