...
public class Link {
	...
	public Link(String laddr, String lport, String raddr, String rport)
		...
		try {
			...
			String d = localaddr.getHostName() + " - "
					+ localaddr.getCanonicalHostName() + ":" + localport;
			throw new NodeException(NodeException.SOCKETERROR, d);
		}

		receiver = new Linkreceiver(this, socket); // Thread recepcion
		sender = new Linksender(this, socket, remoteaddr, remoteport); // Thread
																		// emision
		System.out.println("Creado link, origen: " + laddr + ":" + lport
				+ "  Destino: " + raddr + ":" + rport);
	}
	...
	// A partir de los strings direccion-port obtiene las direcciones
	// (InetAddress y ports)
