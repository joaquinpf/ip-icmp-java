...
public class IpAddress {
  ...
  // Convierte de notacion decimal con puntos a IpAddress (4 bytes). Genera excepcion si direccion no valida
  public IpAddress(String dottedDecimal) throws AddressException {
    String[] partes;
    Integer[] n = new Integer[4];
    partes = dottedDecimal.split("P");
    if(partes.length != 4) throw new AddressException("Bad IP address");
    for(int i=0;i<4;i++) n[i] = new Integer(partes[i]);
    for(int i=0;i<4;i++) if(n[i]<0 || n[i]>255) throw new AddressException("Bad IP address");
  ...
  public IpAddress(Integer x) throws AddressException {
    if((x<0) || (x >= 0xFFFFFFFF) ) throw new AddressException("Bad IP Address");
    IpAddress = x;
  }

  // Convierte cuatro bytes a una IpAddress
  public IpAddress(byte a, byte b, byte c, byte d) {
    int cc = (int) ( ((a<<24)&0x00ff000000)+ ((b<<16)&0x00ff0000) + ((c<<8)&0x00ff00)+ (d&0x00ff));
  ...
  public boolean equals(IpAddress f) {
    boolean r = false;
    if(this.IpAddress.equals(f.IpAddress)) r = true;
    return r;
  }
  ...
  public int toInt() {return IpAddress.intValue();}
