...
import NetworkProtocols.*;
  ...
  public class RoutingEntry {
    ...
    RoutingEntry(IpAddress dst, Mask m, boolean type, IpAddress nh, Interface ifc) {
    Destination = dst;
    mask = m;
    direct = type;
    nextHop = nh;
  ...
  // Devuelve la cantidad de bits de matching de la entrada con la direccion parametro
  // Si no hay matching, devuelve -1. Si no hay matching con ninguna entrada pero existe
  // default route, devuelve matching 0 (notar que el chequeo da  true para default)
  public int getmatching(IpAddress dest) {
    int mat = -1;
    if( (dest.toInt() & mask.toInt()) == Destination.toInt()) mat = mask.toPrefix();
    return mat;
  }

  // Devuelve true si es la defayult entry
  public boolean isdefault() {
    if((Destination.toInt() == 0) && (mask.toInt() == 0 )) return true; else return false;
  }

  // Devuelve true si la entrada es igual
  public boolean equals(IpAddress dst, Mask m) {
    if(Destination.equals(dst)  && mask.equals(m)) return true; else return false;
  }
  ...
  public String toString() {
    String di = "I";
    if(direct) di = "D";
    String st = "("+Destination.toString()+" , "+mask.toString()+" , "+di+" , "+nextHop.toString()+" , "+interface1+")";
    return(st);
  }
  
  public IpAddress getDestination() {return Destination;}  
  
  public IpAddress getNextHop() {return nextHop;}
  
  public Mask getMask() {return mask;} 
  
  public Interface getInterface() {return interface1;}
  
  public boolean getType() {return direct;}
